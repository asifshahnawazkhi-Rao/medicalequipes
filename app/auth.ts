...
export function signInWithPassword(email: string, password: string) {
  return authRequest<AuthSession>("token?grant_type=password", { email, password });
}

export function signUpWithPassword(email: string, password: string) {
  const redirectTo = typeof window !== "undefined" ? `${window.location.origin}/dashboard` : undefined;
  return authRequest<AuthSession>("signup", { email, password, options: { email_redirect_to: redirectTo } });
}

export function sendPasswordReset(email: string) {
  const redirectTo = typeof window !== "undefined" ? `${window.location.origin}/reset-password` : undefined;
  return authRequest<{ message?: string }>("recover", { email, redirect_to: redirectTo });
}

export function saveSession(session: AuthSession) {
  if (typeof window === "undefined") return;
  window.localStorage.setItem(sessionKey, JSON.stringify(session));
  document.cookie = `${cookieName}=${session.access_token}; path=/; max-age=${session.expires_in ?? 3600}; SameSite=Lax; secure`;
}
