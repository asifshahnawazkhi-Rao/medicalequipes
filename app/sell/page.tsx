  ...
  return;
    ...
    try {
      ...
      if (!/^03\d{9}$/.test(localPhone)) {
        throw new Error("Enter an 11-digit Pakistan mobile number starting with 03.");
      }
      values.title = `${brand} ${model}`;
      values.contactPhone = `+92${localPhone.slice(1)}`;
      if (!priceOnRequest && Number(values.price) <= 0) throw new Error("Price must be greater than zero.");
