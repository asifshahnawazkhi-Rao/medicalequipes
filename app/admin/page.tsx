...
export default function AdminPage() {
  ...
  return <main className="adminDashboardPage">
    ...
    <div className="container adminDashboardContainer">
      ...
      {section === "sellers" && <section className="adminPanel"><div className="adminPanelHead"><div><h2>Seller Management</h2><p>Approve, reject or review registered sellers.</p></div><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search sellers..." /></div>{filters(["all", "pending", "approved", "rejected"], sellerFilter, setSellerFilter)}<div className="adminTableWrap"><table className="adminTable"><thead><tr><th>Seller</th><th>Contact</th><th>City</th><th>Status</th><th>Verification</th><th>Actions</th></tr></thead><tbody>{visibleSellers.map((x) => <tr key={x.id}><td><button className="adminSellerLink" type="button" onClick={() => setSelectedSeller(x)}><strong>{x.businessName || x.fullName || "Seller"}</strong><small>{x.fullName}</small></button></td><td>{x.phone || "—"}</td><td>{x.city || "—"}</td><td><span className={`adminStatus ${x.status}`}>{x.status}</span></td><td>{x.cardUrl ? <a href={x.cardUrl} target="_blank" rel="noreferrer">View card</a> : "Not provided"}</td><td><div className="adminRowActions"><button type="button" onClick={() => setSelectedSeller(x)}>View Details</button>{x.status !== "approved" && <button disabled={busyId === x.id} onClick={() => sellerStatus(x.id, "approved")}>Approve</button>}{x.status !== "rejected" && <button disabled={busyId === x.id} onClick={() => sellerStatus(x.id, "rejected")}>Reject</button>}</div></td></tr>)}</tbody></table>{!visibleSellers.length && <div className="adminEmpty">No sellers found.</div>}</div></section>}

      {selectedSeller && <div className="adminSellerModal" role="dialog" aria-modal="true" aria-labelledby="admin-seller-title" onClick={() => setSelectedSeller(null)}><section className="adminSellerDetails" onClick={(event) => event.stopPropagation()}><button className="adminSellerClose" type="button" aria-label="Close seller details" onClick={() => setSelectedSeller(null)}>×</button><span className="eyebrow">SELLER DETAILS</span><h2 id="admin-seller-title">{selectedSeller.businessName || selectedSeller.fullName || "Seller"}</h2><div className="adminSellerDetailGrid"><div><span>Contact person</span><strong>{selectedSeller.fullName || "Not provided"}</strong></div><div><span>Phone</span><strong>{selectedSeller.phone || "Not provided"}</strong></div><div><span>City</span><strong>{selectedSeller.city || "Not provided"}</strong></div><div><span>Status</span><strong className={`adminStatus ${selectedSeller.status}`}>{selectedSeller.status}</strong></div><div><span>Active listings</span><strong>{listings.filter((listing) => listing.sellerId === selectedSeller.id && listing.status === "active").length}</strong></div><div><span>Total listings</span><strong>{listings.filter((listing) => listing.sellerId === selectedSeller.id).length}</strong></div></div><div className="adminSellerDetailActions">{selectedSeller.cardUrl && <a href={selectedSeller.cardUrl} target="_blank" rel="noreferrer">View Visiting Card</a>}{selectedSeller.status === "approved" && <a href={`/seller/${selectedSeller.id}`} target="_blank" rel="noreferrer">Open Public Seller Profile</a>}</div></section></div>}
